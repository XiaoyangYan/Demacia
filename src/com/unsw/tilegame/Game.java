...
public class Game{
	...
	public void tick() {
		...
		if (!handler.getWorld().getPlayer().isActive()) {
		...
		} else if (gameIsWinForMapTreasure() && worldId == 2) {
			new WinSceneController(stage).handleMousePressOne();
		} else if (gameIsWinForMapStone() && worldId == 3) {
			new WinSceneController(stage).handleMousePressOne();
		} else if (worldId == 7) {
			if (gameIsWinForSelfDesign()) {
				World world = new World(getHandler(),"res/worlds/world6.txt",7);
        		Player player = new Player(world.getHandler(),100,100);
        		EntityManager entityManager = new EntityManager(world.getHandler(),player);
	...
	public boolean gameIsWinForSelfDesign() {
		if (Player.getTreasure() == 10 || Rock.getRockSwitch() == 2 || isWin || defineEnemyCleared()) {
			return true;
		}
		return false;
	}
	
	public boolean defineEnemyCleared() {
